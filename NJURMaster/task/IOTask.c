...
{				
  ...
  //          MPU6050_getlastMotion6(&ax, &ay, &az, &gx, &gy, &gz);   //发送的是原始数据
   //         HMC58X3_getlastValues(&hx, &hy, &hz);																												//LFM
            Sensor_Info_Send(ax, ay, az, gx, gy, gz, hx, hy, hz);
            upload_type = REHMC; //如果正在进行磁标定，则发送当前磁力计标定值
        }break;
        case REHMC:
        {
//            Mag_Cali_Info_Send(MagMaxMinData.MaxMagX,MagMaxMinData.MaxMagY,MagMaxMinData.MaxMagZ,MagMaxMinData.MinMagX,MagMaxMinData.MinMagY,MagMaxMinData.MinMagZ);//LFM
